...
async function generateFacilityReportData(userId: string, facilities: any[], loans: any[], banks: any[], startDate?: string, endDate?: string) {
  ...
  // Group facilities by bank
  const bankExposures = new Map();
  let totalExposure = 0;
  ...
  facilities.forEach(facility => {
    ...
    if (!bankExposures.has(bankId)) {
      bankExposures.set(bankId, {
        bankName,
        totalLimit: 0,
        totalOutstanding: 0,
        facilities: []
    ...
    const bankData = bankExposures.get(bankId);
    bankData.totalLimit += creditLimit;
    bankData.totalOutstanding += outstanding;
    bankData.facilities.push({
      id: facility.id,
      name: facility.facilityName || `${bankName} Facility`,
      type: facility.facilityType,
      limit: creditLimit,
      outstanding: outstanding,
      available: creditLimit - outstanding,
      utilization: creditLimit > 0 ? (outstanding / creditLimit * 100).toFixed(1) : '0'
    });
  ...
  return {
    ...
    companyName: 'KAM Office', // Could be made dynamic
    totalExposure,
    bankExposures: Array.from(bankExposures.values()),
    summary: {
      totalBanks: bankExposures.size,
      totalFacilities: facilities.length,
      totalActiveLoans: loans.length,
      avgUtilization: facilities.length > 0 ? 
        (facilities.reduce((sum, f) => {
          const limit = parseFloat(f.creditLimit || '0');
          const outstanding = parseFloat(f.outstandingAmount || '0');
          return sum + (limit > 0 ? outstanding / limit * 100 : 0);
        }, 0) / facilities.length).toFixed(1) : '0'
    }
  };
}
...
async function generatePDFReport(reportData: any): Promise<Buffer> {
  ...
  doc.text(reportData.companyName, 20, 25);
  
  doc.setFontSize(16);
  doc.text(reportData.reportTitle, 20, 35);
  
  doc.setFontSize(10);
  doc.text(reportData.reportDate, 20, 45);
  
  // Total Exposure Summary
  doc.setFontSize(14);
  doc.text('Total Exposure Summary:', 20, 60);
  
  const summaryData = [];
  reportData.bankExposures.forEach((bank: any) => {
    summaryData.push([
      bank.bankName,
      `SAR ${bank.totalOutstanding.toLocaleString('en-US', { minimumFractionDigits: 0 })}`
    ]);
  });
  summaryData.push([
    'Total Exposure',
    `SAR ${reportData.totalExposure.toLocaleString('en-US', { minimumFractionDigits: 0 })}`
  ]);
  
  (doc as any).autoTable({
    startY: 70,
    head: [['Bank', 'Outstanding Amount']],
    body: summaryData,
    theme: 'grid',
  ...
  reportData.bankExposures.forEach((bank: any, index: number) => {
    if (yPosition > 250) {
      doc.addPage();
      yPosition = 20;
    }
    ...
    doc.setFontSize(14);
    doc.text(`${bank.bankName} Facility Summary:`, 20, yPosition);
    yPosition += 10;
    
    doc.setFontSize(10);
    doc.text(`Total Limit: SAR ${bank.totalLimit.toLocaleString('en-US', { minimumFractionDigits: 0 })}`, 20, yPosition);
    yPosition += 7;
    doc.text(`Total Outstanding: SAR ${bank.totalOutstanding.toLocaleString('en-US', { minimumFractionDigits: 0 })}`, 20, yPosition);
    yPosition += 15;
    
    // Facility details table
    const facilityData = bank.facilities.map((facility: any) => [
      facility.name,
      facility.type || 'Working Capital',
      `SAR ${facility.limit.toLocaleString('en-US', { minimumFractionDigits: 0 })}`,
      `SAR ${facility.outstanding.toLocaleString('en-US', { minimumFractionDigits: 0 })}`,
      `SAR ${facility.available.toLocaleString('en-US', { minimumFractionDigits: 0 })}`,
      `${facility.utilization}%`
    ]);
    
    (doc as any).autoTable({
      startY: yPosition,
      head: [['Facility Name', 'Type', 'Limit', 'Outstanding', 'Available', 'Utilization']],
      body: facilityData,
      theme: 'striped',
...
async function generateExcelReport(reportData: any): Promise<Buffer> {
  ...
  // Summary Sheet
  const summaryData = [
    ['Facility Report Summary'],
    ['Report Date:', reportData.reportDate],
    ['Company:', reportData.companyName],
    [''],
    ['Total Banks:', reportData.summary.totalBanks],
    ['Total Facilities:', reportData.summary.totalFacilities],
    ['Total Active Loans:', reportData.summary.totalActiveLoans],
    ['Average Utilization:', `${reportData.summary.avgUtilization}%`],
    [''],
    ['Bank', 'Total Limit (SAR)', 'Outstanding (SAR)', 'Available (SAR)', 'Utilization (%)']
  ];
  
  reportData.bankExposures.forEach((bank: any) => {
    const utilization = bank.totalLimit > 0 ? (bank.totalOutstanding / bank.totalLimit * 100).toFixed(1) : '0';
    summaryData.push([
      bank.bankName,
      bank.totalLimit,
      bank.totalOutstanding,
      bank.totalLimit - bank.totalOutstanding,
      utilization
    ]);
  });
  ...
  const facilityData = [
    ['Bank', 'Facility Name', 'Type', 'Credit Limit (SAR)', 'Outstanding (SAR)', 'Available (SAR)', 'Utilization (%)']
  ];
  
  reportData.bankExposures.forEach((bank: any) => {
    bank.facilities.forEach((facility: any) => {
      facilityData.push([
        bank.bankName,
        facility.name,
        facility.type || 'Working Capital',
  ...
  XLSX.utils.book_append_sheet(workbook, facilitySheet, 'Facility Details');
  
  return XLSX.write(workbook, { type: 'buffer', bookType: 'xlsx' });
}

async function generateBankExposureReportData(userId: string, exposures: any[], startDate?: string, endDate?: string) {
  // Process exposure data for trend analysis
  const bankTrends = new Map();
  
  exposures.forEach(exposure => {
    if (!exposure.facilityId) { // Only bank-level exposures
      const bankId = exposure.bankId;
      if (!bankTrends.has(bankId)) {
        bankTrends.set(bankId, []);
      }
      bankTrends.get(bankId).push({
        date: exposure.date,
        outstanding: parseFloat(exposure.outstanding),
        creditLimit: parseFloat(exposure.creditLimit)
      });
    }
  });
  
  return {
    reportTitle: 'Bank Exposure Analysis',
    dateRange: `${startDate || 'All time'} to ${endDate || 'Present'}`,
    bankTrends: Array.from(bankTrends.entries()).map(([bankId, trends]) => ({
      bankId,
      trends: trends.sort((a: any, b: any) => new Date(a.date).getTime() - new Date(b.date).getTime())
    }))
  };
}

async function generateBankExposurePDFReport(reportData: any): Promise<Buffer> {
  const doc = new jsPDF();
  
  // Header
  doc.setFontSize(20);
  doc.text('Bank Exposure Analysis', 20, 25);
  ...
  reportData.bankTrends.forEach((bankTrend: any) => {
    bankTrend.trends.forEach((trend: any) => {
      trendsData.push([
        trend.date,
        bankTrend.bankId,
        `SAR ${trend.outstanding.toLocaleString()}`,
        `SAR ${trend.creditLimit.toLocaleString()}`,
        `${((trend.outstanding / trend.creditLimit) * 100).toFixed(1)}%`
      ]);
    });
  });
  ...
  (doc as any).autoTable({
    startY: 50,
    head: [['Date', 'Bank ID', 'Outstanding', 'Credit Limit', 'Utilization']],
    body: trendsData,
    theme: 'grid',
    styles: { fontSize: 9 },
...
async function generateBankExposureExcelReport(reportData: any): Promise<Buffer> {
  const workbook = XLSX.utils.book_new();
  
  const trendsData = [
    ['Bank Exposure Trends'],
    ['Period:', reportData.dateRange],
    [''],
    ['Date', 'Bank ID', 'Outstanding (SAR)', 'Credit Limit (SAR)', 'Utilization (%)']
  ];
  ...
  XLSX.utils.book_append_sheet(workbook, trendsSheet, 'Exposure Trends');
