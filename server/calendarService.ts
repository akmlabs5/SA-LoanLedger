...
export class CalendarService {
  ...
  static generateICSFile(reminder: LoanReminder, loan: Loan, userEmail: string): string {
    const event: CalendarEvent = {
      uid: `loan-reminder-${reminder.id}@morounaloans.com`,
      ...
      organizer: 'noreply@morounaloans.com',
      attendee: userEmail,
    };
  ...
  private static generateEventDescription(reminder: LoanReminder, loan: Loan): string {
    ...
    return [
      ...
      ``,
      `Generated by Morouna Loans Platform`
    ].join('\n');
  }
  ...
  private static buildICSContent(event: CalendarEvent): string {
    ...
    return [
      'BEGIN:VCALENDAR',
      'VERSION:2.0',
      'PRODID:-//Morouna Loans//Loan Reminder//EN',
      'CALSCALE:GREGORIAN',
      'METHOD:PUBLISH',
      'BEGIN:VEVENT',
      `UID:${event.uid}`,
  ...
  private static buildBulkICSContent(events: CalendarEvent[]): string {
    ...
    const header = [
      'BEGIN:VCALENDAR',
      'VERSION:2.0',
      'PRODID:-//Morouna Loans//Loan Reminders//EN',
      'CALSCALE:GREGORIAN',
      'METHOD:PUBLISH'
    ];
